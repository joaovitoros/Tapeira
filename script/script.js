...
var missaoColeta = 500,
  missaoColetaAtual = 0.0; //Gold necessario para completar a missão "Coleta de gold"
var missaoGolpe = 100,
  missaoGolpeAtual = 0; //Quantidade de golpes necessarios para concluir a missão "Golpes"
var missaoCacaMugs = 50,
  missaoCacaMugsAtual = 0; //Quantidade de mugs necessarios para completar a missão "Caça aos Mugs"
var missaoTempo = 10000,
  missaoTempoAtual = 0; //tempo em milissegundos necessarios para concluir a missão "Tempo"
...
function AbreConfig() {
  if (
    document.getElementById("container-SalvaCarrega").style.visibility ==
    "hidden"
  ) {
    document.getElementById("container-SalvaCarrega").style.visibility =
      "visible";
...
function Bater(inimigo, validaDano) {
  ...
  if (inimigosDerrotados >= qtdInimigosAndar) {
    if (document.getElementById("manterAndar").checked == true && andar > 4) {
      ...
      totalGold =
        totalGold + (andar * mulGold + vidaAndar * mulGold * mulGoldAvanco);
      document.getElementById("contGold").innerHTML = gold.toFixed(2);
      document.getElementById("contAndar").innerHTML = andar;
      document.getElementById("titulo").innerHTML =
        "Caverna (Andar: " + andar + ")";
...
function AtualizaQTDHabildiade1() {
  ...
  document.getElementById("BaraQTDHab1").style.height =
    Math.round(porcentagem) + "px";
...
function MostraStatus() {
  var msg =
    "Dano: " +
    danoJogador.toFixed(2) +
    "\nMultiplicador de Gold: " +
    mulGold.toFixed(2) +
    "\nGold por inimigo derrotado: " +
    (andar * mulGold).toFixed(2) +
    "\nTotal de gold coletado: " +
    totalGold.toFixed(2) +
    "\nBonus de gold a cada andar:" +
    (andar * mulGold + vidaAndar * mulGold * mulGoldAvanco).toFixed(2) +
    "\nChance de Avanço Rapido: " +
    (avanco * 100).toFixed(2) +
    "%" +
    "\nQuantidade de avanço rapido: " +
    qtdAvanco +
    "\nDano critico em: " +
    danoCritJogador.toFixed(2) +
    "\nChance de dano critico em: " +
    (chanceCrit * 100).toFixed(2) +
    "%" +
    "\nDPS de companheiros: " +
    danoComp.toFixed(2) +
    "\nGoldPS de companheiros: " +
    goldCompanheiro.toFixed(2) +
    "\nTempo bonus de companheiro: " +
    tempoEsperaCompanheiro +
    " seg\nVida Mug - " +
    (subVidaInimigo * 100).toFixed(2) +
    " %\nChance de surgir bau: " +
    (chanceBau * 100).toFixed(2) +
    "%\nChance de esmeralda em bau: " +
    (chanceEsmeraldaBau * 100).toFixed(2) +
    "%";
...
function AvancoInimigos() {
  ...
  for (i = 1; i <= limiteInimigos; i++) {
    if (i == 1 || i == 2) {
      try {
        document.getElementById("inimigo" + i).style.left =
          10 + -(tempoAvancoInimigos/80) * 4 + "%";
      } catch (error) {
        console.log(error);
      }
    } else {
  ...
  if (tempoAvancoInimigos <= 0) {
    if (andar > 1) {
      ...
      CriarInimigos();
      MostraInfo(
        "Dica: Se estiver com dificudlade para avançar compre o item Avanço rapido para ter mais faclidade durante os andares"
      );
    }
    tempoAvancoInimigos = 120;
  }
...
function VoltaAndar() {
  if (andar >= andarVolta) {
    ...
    MostraInfo(
      "É necessario chegar no andar " + andarVolta + " para poder voltar"
    );
  }
}
...
function Conquistas() {
  if (totalDerrotados >= progressoConquistaDano) {
    ...
    if (validaConquista == 1) {
      ...
      goldAux =
        gold + (andar * mulGold + vidaAndar * mulGold * mulGoldAvanco) * 2;
...
function MostraConquista() {
  ...
  conquista =
    "Situação da conquista de dano atual\n" +
    totalDerrotados +
    " inimigos derrotados de " +
    progressoConquistaDano +
    " necessarios para receber bonus de ";
  if (validaConquista == 1) {
    conquista +=
      "dano (" +
  ...
  conquista +=
    ...
    " gold coletado de " +
    progressoConquistaGold +
    " necessarios para receber bonus (" +
  ...
  if (andar < 10) {
    ...
    if (missaoAtual == 1) {
      msg +=
        ...
        missaoColeta +
        " (" +
        (missaoColetaAtual * 100) / missaoColeta +
        "%)";
      ...
      msg +=
        ...
        missaoGolpe +
        " (" +
        (missaoGolpeAtual * 100) / missaoGolpe +
        "%)";
      ...
      msg +=
        ...
        missaoCacaMugs +
        " (" +
        (missaoCacaMugsAtual * 100) / missaoCacaMugs +
        "%)";
      ...
      msg +=
        ...
        missaoTempo +
        " (" +
        (missaoTempoAtual * 100) / missaoTempo +
        "%)";
    }
    alert(msg);
  }
...
function AbreLoja() {
  if (document.getElementById("Loja").style.visibility == "hidden") {
    ...
    document.getElementById(
      "precoVidaInimigo"
    ).innerHTML = precoVidaInimigo.toFixed(2);
    ...
    document.getElementById("precoQTDAvan").innerHTML = precoQTDAvanco.toFixed(
      2
    );
...
function CompraQTDAvanco() {
  if (lvlQTDAvanco <= 20) {
    if (gold >= precoQTDAvanco) {
      ...
      document.getElementById(
        "precoQTDAvan"
      ).innerHTML = precoQTDAvanco.toFixed(2);
      document.getElementById("lvlQTDAvan").innerHTML = lvlQTDAvanco;
      MostraStatus();
    } else {
      MostraInfo("Voce não tem gold o suficiente para essa compra!");
